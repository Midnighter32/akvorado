...
func assertCSVContainsLines(t testing.TB, addr net.Addr, url string, expected []string) {
	t.Helper()
	resp, err := http.Get("http://" + addr.String() + url)
	if err != nil {
		t.Fatalf("GET %s error:\n%+v", url, err)
	}
	defer resp.Body.Close()

	body, err := io.ReadAll(resp.Body)
	if err != nil {
		t.Fatalf("ReadAll(%s) error:\n%+v", url, err)
	}
	got := string(body)
	for _, line := range expected {
		if !strings.Contains(got, line) {
			t.Fatalf("GET %s missing line:\n%s", url, line)
		}
	}
}
...
func TestNetworksCSVWithGeoip(t *testing.T) {
	...
	t.Run("only GeoIP", func(t *testing.T) {
		...
		assertCSVContainsLines(t, c.d.HTTP.LocalAddr(), "/api/v0/orchestrator/clickhouse/networks.csv", []string{
			"network,name,role,site,region,country,state,city,latitude,longitude,tenant,asn",
			"Brisbane,-27.48159,153.0175",
			"Xiamen,24.47979,118.08187",
			"Asagaya-minami,35.70206,139.63745",
		})
	})
	...
	t.Run("custom networks", func(t *testing.T) {
		...
		assertCSVContainsLines(t, c.d.HTTP.LocalAddr(), "/api/v0/orchestrator/clickhouse/networks.csv", []string{
			"network,name,role,site,region,country,state,city,latitude,longitude,tenant,asn",
			"::ffff:0.80.0.0/112",
			"::ffff:1.0.0.0/116,infra",
			"Brisbane,-27.48159,153.0175",
			"::ffff:1.0.16.64/122,infra",
			"::ffff:1.0.16.66/128,infra",
			"Alfred,",
		})
	})
	...
	t.Run("cleanup old files", func(t *testing.T) {
		...
		if !strings.Contains(strings.Join(got, "\n"), expected[0]) {
			t.Fatalf("Unexpected current temporary file list:\n got: %v\nwant: %v", got, expected)
		}
	})
}
